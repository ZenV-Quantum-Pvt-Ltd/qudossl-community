...
#ifndef QUDO_PQC_DELEGATE
        /*
         * The delegated encap and decap paths derive the matrix themselves from
         * the encodings, so |m| is never read; expanding it here would repeat
         * rank^2 SHAKE128 rejection samplings on every public key parsed.
         */
...
#ifdef QUDO_PQC_DELEGATE
    /*
     * |m| is unread under delegation but shares the single key allocation that
     * ossl_ml_kem_key_dup() copies wholesale, so it must not stay indeterminate.
     */
    memset(key->m, 0, (size_t)vinfo->rank * vinfo->rank * sizeof(*key->m));
#endif
    return 1;
}
...
ML_KEM_KEY *ossl_ml_kem_key_dup(const ML_KEM_KEY *key, int selection)
{
    ...
    case OSSL_KEYMGMT_SELECT_KEYPAIR:
        /*
         * ML-KEM private storage is a single |prvalloc| block that also holds
         * the public vector, so a keypair dup is a private-key dup. Upstream
         * 3.5.7 omits the KEYPAIR case, so EVP_PKEY_dup(ML-KEM) returns NULL
         * (issue #31114, not backported to 3.5.x).
         */
    ...
    /*
     * add_storage() re-homes |rho|/|pkhash| onto a freshly zeroed seedbuf;
     * restore their values from the source key. The initial memdup() copied
     * the struct, but add_storage() cleared that seedbuf copy.
     */
    if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
        memcpy(ret->rho, key->rho, ML_KEM_RANDOM_BYTES);
        memcpy(ret->pkhash, key->pkhash, ML_KEM_PKHASH_BYTES);
    }
...
#ifdef QUDO_PQC_DELEGATE
/*
 * Crypto-layer delegation to qudo-pqc-lib (ADR-0005 / Decision Log D2).
 *
 * Only the algorithm math moves. Everything above this file is upstream
 * OpenSSL and untouched: the provider dispatch tables, EVP, the DER/PEM
 * codecs, TLS group negotiation, and the FIPS self-test machinery. Because
 * self_test_kats.c drives these same ossl_ml_kem_* entry points, OpenSSL's
 * POST and CASTs exercise Qudo's math with OpenSSL's vectors under OpenSSL's
 * state machine -- nothing to bridge, nothing to migrate.
 *
 * BOUNDARY RULE. OpenSSL supplies every FIPS service inside the boundary:
 * POST, CAST, PCT, integrity HMAC, the approved DRBG, AES/SHA/HMAC, the state
 * machine and the approval indicator. qudo-pqc-lib is built MATH_ONLY so none
 * of its own copies exist. Enforced by ci/check-boundary-symbols.sh.
 *
 * RANDOMNESS RULE. Every call below is a SEEDED entry point -- randomness is
 * drawn by OpenSSL from the approved DRBG and passed in. The non-seeded
 * variants must never be used: they draw from qudo-pqc's platform entropy,
 * not the approved DRBG. Note QUDO_KEM_keypair_derand is NOT seeded despite
 * its name; its seed argument is an output.
 *
 * SUPERSEDED UPSTREAM MATH. The FIPS 203 implementation above -- genkey, encap,
 * decap and the ~1400 lines of scalar/vector/NTT helpers they reach -- is dead
 * once this macro is defined. It is deliberately NOT wrapped in #ifndef. Every
 * one of those functions is static within this single translation unit, so at
 * -O3 the compiler proves them unreferenced and never emits them: __text falls
 * from 42280 to 6088 bytes and the NTT root tables appear in no object, in
 * libcrypto and in the FIPS module. Gating would remove zero bytes from the
 * boundary while permanently complicating upstream CVE cherry-picks. The key
 * codec (parse/encode and their helpers) is NOT dead and must stay. ADR-0011.
 */

/*
 * QUDO_KEM handles are small immutable descriptors that QUDO_KEM_new()
 * allocates by copying a static template. They are created per operation
 * rather than cached: the allocation is negligible beside the lattice math,
 * and it avoids a run-once, which is unavailable in the FIPS module build.
 */
static QUDO_KEM *qudo_kem_new_for(const ML_KEM_VINFO *vinfo)
{
    switch (vinfo->evp_type) {
    case EVP_PKEY_ML_KEM_512:
        return QUDO_KEM_new(QUDO_KEM_alg_mlkem_512);
    case EVP_PKEY_ML_KEM_768:
        return QUDO_KEM_new(QUDO_KEM_alg_mlkem_768);
    case EVP_PKEY_ML_KEM_1024:
        return QUDO_KEM_new(QUDO_KEM_alg_mlkem_1024);
    default:
        return NULL;
    }
}
...
/*
 * Delegated keygen. qudo-pqc emits FIPS 203 byte encodings; OpenSSL's
 * ML_KEM_KEY is structured, so the private encoding is handed straight to
 * upstream's own parser. That keeps the codecs OpenSSL's, per D2, and means
 * the resulting key is indistinguishable from one upstream produced.
 *
 * seed is d || z, already drawn from the approved DRBG by the caller.
 */
static int qudo_genkey(const uint8_t *seed, EVP_MD_CTX *mdctx,
    uint8_t *pubenc, ML_KEM_KEY *key)
{
    const ML_KEM_VINFO *vinfo = key->vinfo;
    QUDO_KEM *kem = qudo_kem_new_for(vinfo);
    uint8_t *prvenc = NULL, *pub = NULL;
    int ret = 0;
    ...
    if ((prvenc = OPENSSL_malloc(vinfo->prvkey_bytes)) == NULL
        || (pub = OPENSSL_malloc(vinfo->pubkey_bytes)) == NULL)
        goto end;

    if (QUDO_KEM_keypair_from_seed(kem, pub, prvenc, seed) != QUDO_KEM_SUCCESS) {
        ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_GENERATE_KEY);
        goto end;
    }

    /* Populate the OpenSSL key from the encoded private key. */
    if (!parse_prvkey(prvenc, mdctx, key)) {
        ERR_raise(ERR_LIB_PROV, PROV_R_INVALID_KEY);
        goto end;
    }

    /*
     * parse_prvkey() populates from the NIST private-key encoding, which does
     * not carry |d| -- it leaves key->d NULL, marking the key as "no seed
     * available". A generated key must still be exportable in seed form, so
     * restore the seed halves exactly as upstream genkey() does: |z| for
     * implicit rejection, and |d| only when the caller asked to retain it.
     * Without this, seed-format encoding is silently lost and the ML-KEM
     * codec tests fail.
     */
    memcpy(key->z, seed + ML_KEM_RANDOM_BYTES, ML_KEM_RANDOM_BYTES);
    key->d = key->z + ML_KEM_RANDOM_BYTES;
    ...
    if (pubenc != NULL)
        memcpy(pubenc, pub, vinfo->pubkey_bytes);
    ret = 1;

 end:
    /* The private encoding is secret material; the public key is not. */
    OPENSSL_clear_free(prvenc, vinfo->prvkey_bytes);
    OPENSSL_free(pub);
    QUDO_KEM_free(kem);
    return ret;
}
...
/*
 * Delegated encapsulation. The shared secret and ciphertext are produced
 * entirely by qudo-pqc from the caller-supplied entropy, so this path is
 * deterministic given (entropy, public key).
 */
static int qudo_encap(uint8_t *ctext, uint8_t *secret,
    const uint8_t *entropy, const ML_KEM_KEY *key)
{
    const ML_KEM_VINFO *vinfo = key->vinfo;
    QUDO_KEM *kem = qudo_kem_new_for(vinfo);
    uint8_t *pubenc = NULL;
    ...
    if ((pubenc = OPENSSL_malloc(vinfo->pubkey_bytes)) == NULL) {
        QUDO_KEM_free(kem);
        return 0;
    }

    if (!ossl_ml_kem_encode_public_key(pubenc, vinfo->pubkey_bytes, key))
        ERR_raise(ERR_LIB_PROV, PROV_R_INVALID_KEY);
    else if (QUDO_KEM_encaps_derand(kem, ctext, secret, pubenc, entropy)
             != QUDO_KEM_SUCCESS)
        ERR_raise(ERR_LIB_PROV, PROV_R_INVALID_KEY);
    else
        ret = 1;
...
/*
 * Delegated decapsulation. Implicit rejection on a malformed ciphertext is
 * handled inside qudo-pqc per FIPS 203, matching upstream's behaviour: a
 * pseudorandom secret is returned rather than an error.
 */
static int qudo_decap(uint8_t *secret, const uint8_t *ctext,
    const ML_KEM_KEY *key)
{
    const ML_KEM_VINFO *vinfo = key->vinfo;
    QUDO_KEM *kem = qudo_kem_new_for(vinfo);
    ...
    /*
     * Note: a decap failure here is a hard error, not implicit rejection.
     * FIPS 203 implicit rejection is handled inside qudo-pqc, which returns
     * success with a pseudorandom secret for a malformed ciphertext.
     */
    if (!ossl_ml_kem_encode_private_key(prvenc, vinfo->prvkey_bytes, key))
        ERR_raise(ERR_LIB_PROV, PROV_R_MISSING_KEY);
    else if (QUDO_KEM_decaps(kem, secret, ctext, prvenc) != QUDO_KEM_SUCCESS)
        ERR_raise(ERR_LIB_PROV, PROV_R_INVALID_KEY);
    else
        ret = 1;
...
int ossl_ml_kem_genkey(uint8_t *pubenc, size_t publen, ML_KEM_KEY *key)
{
    ...
    if (add_storage(OPENSSL_malloc(vinfo->prvalloc), 1, key))
#ifdef QUDO_PQC_DELEGATE
        ret = qudo_genkey(seed, mdctx, pubenc, key);
#else
    ...
    const ML_KEM_KEY *key)
{
     ...
     */
#ifdef QUDO_PQC_DELEGATE
    ret = qudo_encap(ctext, shared_secret, entropy, key);
#else
    ...
    const ML_KEM_KEY *key)
{
     ...
     */
#ifdef QUDO_PQC_DELEGATE
    ret = qudo_decap(shared_secret, ctext, key);
#else
