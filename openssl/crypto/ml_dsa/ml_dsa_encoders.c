...
int ossl_ml_dsa_keypair_decode(ML_DSA_KEY *key,
    ...
    const uint8_t *sk, size_t sk_len)
{
    ...
    size_t i, k = params->k, l = params->l;
    uint8_t input_tr[ML_DSA_TR_BYTES];
    uint8_t pk_rho[ML_DSA_RHO_BYTES];
    EVP_MD_CTX *ctx = NULL;
    PACKET pkt;
    int ret = 0;
    ...
    if (key->priv_encoding != NULL || key->pub_encoding != NULL)
        return 0;
    if (pk_len != params->pk_len || sk_len != params->sk_len)
        return 0;
    if (!ossl_ml_dsa_key_pub_alloc(key) || !ossl_ml_dsa_key_priv_alloc(key))
        return 0;
    ...
    if (!PACKET_buf_init(&pkt, sk, sk_len)
        || !PACKET_copy_bytes(&pkt, key->rho, sizeof(key->rho))
        || !PACKET_copy_bytes(&pkt, key->K, sizeof(key->K))
        || !PACKET_copy_bytes(&pkt, input_tr, sizeof(input_tr)))
        goto err;
    ...
    if (!PACKET_buf_init(&pkt, pk, pk_len)
        || !PACKET_copy_bytes(&pkt, pk_rho, sizeof(pk_rho)))
        goto err;
    if (memcmp(pk_rho, key->rho, sizeof(pk_rho)) != 0)
        goto err;
    for (i = 0; i < key->t1.num_poly; ++i)
        if (!poly_decode_10_bits(key->t1.poly + i, &pkt))
            goto err;
    ...
    if ((ctx = EVP_MD_CTX_new()) == NULL)
        goto err;
    if (!shake_xof(ctx, key->shake256_md, pk, pk_len, key->tr, sizeof(key->tr)))
        goto err;

    if (memcmp(input_tr, key->tr, sizeof(input_tr)) != 0) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_KEY,
            "%s private key does not match its pubkey part", params->alg);
        goto err;
    }

    if ((key->pub_encoding = OPENSSL_memdup(pk, pk_len)) == NULL
...
err:
    if (!ret) {
        /*
         * ossl_ml_dsa_key_reset() drops the seed unconditionally, but the seed
         * belongs to the caller here and its lifetime is governed by
         * ML_DSA_KEY_RETAIN_SEED. Hold it aside so only this function's own
         * allocations are released.
         */
        uint8_t *seed = key->seed;
    ...
    EVP_MD_CTX_free(ctx);
    OPENSSL_cleanse(input_tr, sizeof(input_tr));
    return ret;
}
