...
#ifdef QUDO_PQC_DELEGATE
/*
 * Crypto-layer delegation (ADR-0005 / D2). OpenSSL has already applied the
 * FIPS 204 message encoding -- context string, pure vs pre-hash, or the
 * externally supplied mu -- so only the raw sign/verify math is delegated.
 * |prefix| is therefore NULL: the prefix is already inside |encoded_msg|.
 * |msg_is_mu| selects qudo's external-mu mode.
 *
 * Randomness for the hedged variant arrives in |rnd|, drawn by OpenSSL from
 * the approved DRBG; a NULL |rnd| means deterministic signing.
 */
static QUDO_MLDSA *qudo_mldsa_new_for_sig(const ML_DSA_KEY *key)
{
...
static int qudo_sign(const ML_DSA_KEY *priv, int msg_is_mu,
    const uint8_t *encoded_msg, size_t encoded_msg_len,
    const uint8_t *rnd, size_t rnd_len, uint8_t *out_sig)
{
    QUDO_MLDSA *sig;
    const uint8_t *sk;
    size_t sig_len = priv->params->sig_len;
    int ret = 0;

    /*
     * qudo's sign_internal takes |rnd| without a length and assumes the FIPS
     * 204 rho'' input is ML_DSA_ENTROPY_LEN bytes. Every current caller passes
     * exactly that, but assert it rather than silently truncate or over-read
     * if upstream ever varies the length.
     */
    if (rnd == NULL || rnd_len != ML_DSA_ENTROPY_LEN) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_KEY_LENGTH,
            "signing entropy is %zu bytes, expected %d", rnd_len,
            ML_DSA_ENTROPY_LEN);
        return 0;
    }

    /*
     * external-mu: |encoded_msg| is the 64-byte mu itself and qudo copies
     * ML_DSA_MU_BYTES from it unconditionally. Upstream (the non-delegated
     * path) rejects a wrong-length mu; the provider does not re-check, so the
     * adapter must, or a short caller buffer is over-read and signed.
     */
    if (msg_is_mu && encoded_msg_len != ML_DSA_MU_BYTES) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_KEY_LENGTH,
            "external-mu is %zu bytes, expected %d", encoded_msg_len,
            ML_DSA_MU_BYTES);
        return 0;
    }

    if ((sig = qudo_mldsa_new_for_sig(priv)) == NULL) {
        ERR_raise(ERR_LIB_PROV, PROV_R_KEY_SETUP_FAILED);
        return 0;
    }
    if ((sk = ossl_ml_dsa_key_get_priv(priv)) == NULL)
        ERR_raise(ERR_LIB_PROV, PROV_R_MISSING_KEY);
    else if (QUDO_MLDSA_sign_internal(sig, out_sig, &sig_len,
                 encoded_msg, encoded_msg_len, NULL, 0, rnd, sk,
                 msg_is_mu) != QUDO_MLDSA_SUCCESS
...
static int qudo_verify(const ML_DSA_KEY *pub, int msg_is_mu,
    ...
    const uint8_t *sig_enc, size_t sig_enc_len)
{
    ...
    if (sig_enc_len != pub->params->sig_len)
        return 0;   /* wrong-size signature: verification failure, not an error */
    /* external-mu: reject a wrong-length mu before qudo over-reads it. */
    if (msg_is_mu && msg_enc_len != ML_DSA_MU_BYTES)
        return 0;
    if ((sig = qudo_mldsa_new_for_sig(pub)) == NULL) {
        ERR_raise(ERR_LIB_PROV, PROV_R_KEY_SETUP_FAILED);
        return 0;
    }
    pk = ossl_ml_dsa_key_get_pub(pub);
    if (pk != NULL
        && QUDO_MLDSA_verify_internal(sig, sig_enc, sig_enc_len,
               msg_enc, msg_enc_len, NULL, 0, pk,
               msg_is_mu) == QUDO_MLDSA_SUCCESS)
        ret = 1;
...
#ifdef QUDO_PQC_DELEGATE
        ret = qudo_sign(priv, msg_is_mu, m, m_len, rand, rand_len, sig);
#else
...
#ifdef QUDO_PQC_DELEGATE
    ret = qudo_verify(pub, msg_is_mu, m, m_len, sig, sig_len);
#else
