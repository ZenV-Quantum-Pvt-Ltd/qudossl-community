...
#ifdef QUDO_PQC_DELEGATE
/*
 * Delegated pairwise consistency test (FIPS 140-3 Section 9.4).
 *
 * Upstream recomputes t1/t0 from s1/s2 and compares against the public
 * encoding, which is OpenSSL's own lattice math. Under crypto-layer delegation
 * that would leave a second ML-DSA implementation inside the boundary, and it
 * would stop working once the upstream math is compiled out.
 *
 * A sign-then-verify round-trip is the canonical PCT for a signature key pair,
 * exercises the delegated implementation end to end, and is inexpensive for
 * ML-DSA (unlike SLH-DSA, where signing costs seconds and the key-recomputation
 * form is retained).
 */
int ossl_ml_dsa_key_pairwise_check(const ML_DSA_KEY *key)
{
    static const uint8_t pct_msg[] = "ML-DSA pairwise consistency test";
    uint8_t rnd[ML_DSA_ENTROPY_LEN];
    uint8_t *sig = NULL;
    size_t sig_len = 0;
    int ret = 0;

    if (key->pub_encoding == NULL || key->priv_encoding == NULL)
        return 0;

    if ((sig = OPENSSL_malloc(key->params->sig_len)) == NULL)
        return 0;

    /* Deterministic variant: rnd = 0^32, per FIPS 204. */
    memset(rnd, 0, sizeof(rnd));

    if (ossl_ml_dsa_sign(key, 0, pct_msg, sizeof(pct_msg), NULL, 0,
            rnd, sizeof(rnd), 0, sig, &sig_len, key->params->sig_len)
        && ossl_ml_dsa_verify(key, 0, pct_msg, sizeof(pct_msg), NULL, 0, 0,
               sig, sig_len))
        ret = 1;
    else
 ...
 *
 * Only the algorithm math moves. OpenSSL keeps the key struct, every codec,
 * the provider layer and the FIPS self-tests -- so its POST, CASTs and the
 * pairwise consistency test exercise Qudo's math through these same entry
 * points, with nothing to bridge.
 *
 * BOUNDARY RULE. OpenSSL supplies every FIPS service inside the boundary.
 * qudo-pqc-lib is built MATH_ONLY so none of its own copies exist.
 * RANDOMNESS RULE. Seed material is drawn by OpenSSL from the approved DRBG
 * and passed in; the non-seeded qudo entry points must never be used.
 */
static QUDO_MLDSA *qudo_mldsa_new_for(const ML_DSA_KEY *key)
{
    /* Names match OpenSSL's exactly: "ML-DSA-44" / "-65" / "-87". */
    const char *alg = key->params->alg;

    return alg == NULL ? NULL : QUDO_MLDSA_new(alg);
}

/*
 * Delegated keygen. qudo emits both FIPS 204 byte encodings; the OpenSSL key is
 * then populated by upstream's own decoders. Keeping the codecs OpenSSL's is
 * what D2 requires.
 */
static int qudo_keygen(ML_DSA_KEY *out)
{
    const ML_DSA_PARAMS *params = out->params;
    QUDO_MLDSA *sig = NULL;
    uint8_t *pk = NULL, *sk = NULL;
    uint8_t seed_copy[ML_DSA_SEED_BYTES];
    int retain, ret = 0;

    retain = (out->prov_flags & ML_DSA_KEY_RETAIN_SEED) != 0;

    if (out->seed == NULL) {
        ERR_raise(ERR_LIB_PROV, PROV_R_MISSING_KEY);
        goto end;
    }
    if ((sig = qudo_mldsa_new_for(out)) == NULL) {
        ERR_raise(ERR_LIB_PROV, PROV_R_KEY_SETUP_FAILED);
        goto end;
    }

    /*
     * qudo consumes the seed as an input buffer, so hand it a copy rather than
     * the key's own, which must survive untouched for ML_DSA_KEY_RETAIN_SEED.
     */
    memcpy(seed_copy, out->seed, ML_DSA_SEED_BYTES);

    if ((pk = OPENSSL_malloc(params->pk_len)) == NULL
        || (sk = OPENSSL_malloc(params->sk_len)) == NULL)
        goto end;

    if (QUDO_MLDSA_keypair_internal(sig, pk, sk, seed_copy) != QUDO_MLDSA_SUCCESS) {
        ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_GENERATE_KEY);
        goto end;
    }

    /* Populates the structured key, both encodings and tr. */
    if (!ossl_ml_dsa_keypair_decode(out, pk, params->pk_len,
                                    sk, params->sk_len)) {
        ERR_raise(ERR_LIB_PROV, PROV_R_INVALID_KEY);
        goto end;
    }
     ...
     * RETAIN_SEED is clear, on both the success and the failure path. Mirror
     * upstream exactly so the zeroization trigger is identical either way.
     */
    if (out->seed != NULL && !retain) {
        OPENSSL_clear_free(out->seed, ML_DSA_SEED_BYTES);
        out->seed = NULL;
    }
    OPENSSL_cleanse(seed_copy, sizeof(seed_copy));
    OPENSSL_free(pk);
    if (sk != NULL)
...
#ifdef QUDO_PQC_DELEGATE
# define ML_DSA_KEYGEN(k) qudo_keygen(k)
#else
# define ML_DSA_KEYGEN(k) keygen_internal(k)
#endif
    if (sk == NULL) {
        ret = ML_DSA_KEYGEN(out);
    } else {
        if ((ret = ML_DSA_KEYGEN(out)) != 0
