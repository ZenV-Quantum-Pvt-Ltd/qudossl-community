#!/bin/bash
# Run all ML-DSA fuzzing harnesses
#
# Usage:
#   cd qudo-mldsa
...
echo "================================================================"
echo "  ML-DSA Fuzzing (${DURATION}s per target)"
echo "================================================================"
...
for target in fuzz_mldsa_der_import fuzz_mldsa_pem_import fuzz_mldsa_sig_ops; do
    if [ ! -f "${FUZZ_DIR}/${target}" ]; then
        echo "SKIP: ${target} (not built)"
        continue
    fi

    corpus_name=$(echo $target | sed 's/fuzz_mldsa_/corpus_/' | sed 's/_import//' | sed 's/_ops//')
