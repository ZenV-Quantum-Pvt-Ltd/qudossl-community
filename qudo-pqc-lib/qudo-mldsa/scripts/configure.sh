#!/bin/bash
# QUDO_MLDSA Build Configuration Helper
...
print_header() {
    echo -e "${BLUE}════════════════════════════════════════════════════════════${NC}"
    echo -e "${BLUE}  QUDO_MLDSA Build Configuration Helper${NC}"
    echo -e "${BLUE}════════════════════════════════════════════════════════════${NC}"
}
...
print_help() {
    ...
    echo "  --no-openssl       Don't use OpenSSL (disables DER/PEM serialization)"
...
list_options() {
    ...
    if [ -d "$PROJECT_ROOT/build" ]; then
        ...
        echo ""
        cmake -LA "$PROJECT_ROOT/build" 2>/dev/null | grep -E "^(BUILD_|ENABLE_|MLDSA_|CMAKE_BUILD_TYPE)" || true
    else
        ...
        echo "MLDSA_BUILD_ONLY_LIB:BOOL=OFF"
        echo "MLDSA_EMBEDDED_BUILD:BOOL=OFF"
        echo "MLDSA_USE_OPENSSL:BOOL=ON"
        echo "MLDSA_STRICT_WARNINGS:BOOL=OFF"
        echo "MLDSA_DIST_BUILD:BOOL=ON"
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --no-tests)
            ...
            ;;
        --lib-only)
            CMAKE_ARGS="$CMAKE_ARGS -DMLDSA_BUILD_ONLY_LIB=ON"
            ;;
        --with-avx2)
        ...
        --no-neon)
            ...
            ;;
        --embedded)
            CMAKE_ARGS="$CMAKE_ARGS -DMLDSA_EMBEDDED_BUILD=ON"
            ;;
        --no-openssl)
            CMAKE_ARGS="$CMAKE_ARGS -DMLDSA_USE_OPENSSL=OFF"
            ;;
        --strict)
            CMAKE_ARGS="$CMAKE_ARGS -DMLDSA_STRICT_WARNINGS=ON"
            ;;
        --debug)
            BUILD_TYPE="Debug"
            ;;
...
print_header
echo ""
echo -e "${GREEN}Configuring QUDO_MLDSA with:${NC}"
