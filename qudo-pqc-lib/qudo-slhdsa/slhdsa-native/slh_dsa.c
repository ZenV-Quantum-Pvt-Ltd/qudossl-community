...
/* === Constant-time comparison (FIPS 140-3: no timing leaks in verify) */

static int slh_ct_compare(const uint8_t *a, const uint8_t *b, size_t len)
{
  size_t i;
  uint8_t diff = 0;
                     ...
                     uint64_t i_tree, uint32_t i_leaf)
{
  ...
  return slh_ct_compare(node, var->pk_root, prm->n);
}
