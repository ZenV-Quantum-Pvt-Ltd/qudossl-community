...
static void sha2_256_compress_c(void *v)
{
...
/* === Runtime dispatch: SHA-NI (x86_64/IA32) or portable C fallback === */
/* SHA-NI dispatch requires SLH_HAS_SHA_NI to be defined (Linux x86_64 only) */

#if (defined(PLAT_ARCH_X64) || defined(PLAT_ARCH_IA32)) && defined(SLH_HAS_SHA_NI)
#include <cpuid.h>

extern void sha2_256_compress_shani(void *v);
...
static int cpu_has_sha_ni(void)
{
  ...
  if (!__get_cpuid_count(7, 0, &eax, &ebx, &ecx, &edx))
    return 0;
  return (ebx >> 29) & 1; /* CPUID.07H.EBX[bit 29] = SHA */
}
#endif
...
{
#if (defined(PLAT_ARCH_X64) || defined(PLAT_ARCH_IA32)) && defined(SLH_HAS_SHA_NI)
  /* Benign race on first call from multiple threads — both will
     compute the same value from CPUID. Relaxed atomic load/store so
     ThreadSanitizer does not flag the concurrent read/write. */
#if defined(_MSC_VER)
  static volatile long shani = -1;
  long shani_local = shani; /* aligned long is atomic on x86 */
  if (shani_local < 0) {
    shani_local = cpu_has_sha_ni();
    shani = shani_local;
  }
#else
  static int shani = -1;
  int shani_local = __atomic_load_n(&shani, __ATOMIC_RELAXED);
  if (__builtin_expect(shani_local < 0, 0)) {
    shani_local = cpu_has_sha_ni();
    __atomic_store_n(&shani, shani_local, __ATOMIC_RELAXED);
  }
#endif
  if (shani_local)
  {
#ifdef SLH_EXPERIMENTAL
    sha2_256_compress_count++;
#endif
    sha2_256_compress_shani(v);
    return;
  }
#endif
  sha2_256_compress_c(v);
}
