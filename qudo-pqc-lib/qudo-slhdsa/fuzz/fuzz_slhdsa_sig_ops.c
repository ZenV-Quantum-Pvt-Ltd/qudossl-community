...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    case 0: {
        if (payload_len >= sig->length_public_key + sig->length_signature) {
            ...
            const uint8_t *msg
                = payload + sig->length_public_key + sig->length_signature;
            size_t msg_len
                = payload_len - sig->length_public_key - sig->length_signature;
    ...
    case 1: {
        uint8_t pk[SLH_DSA_MAX_PUBLIC_KEY_BYTES];
        uint8_t sk[SLH_DSA_MAX_SECRET_KEY_BYTES];
        if (sig->keypair(pk, sk) == QUDO_SLHDSA_SUCCESS && payload_len > 0) {
            sig->verify(payload, payload_len / 2, payload + payload_len / 2,
                        payload_len - payload_len / 2, pk);
        }
        break;
    }
    case 2: {
        uint8_t pk[SLH_DSA_MAX_PUBLIC_KEY_BYTES];
        uint8_t sk[SLH_DSA_MAX_SECRET_KEY_BYTES];
        uint8_t signature[SLH_DSA_MAX_SIGNATURE_BYTES];
