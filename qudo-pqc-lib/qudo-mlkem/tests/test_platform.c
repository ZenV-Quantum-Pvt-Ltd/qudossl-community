...
static void test_get_platform(void) {
    TEST("QUDO_KEM_get_platform()");
    const char *platform = QUDO_KEM_get_platform();
...
static void test_get_architecture(void) {
    TEST("QUDO_KEM_get_architecture()");
    const char *arch = QUDO_KEM_get_architecture();
...
static void test_has_avx2(void) {
    TEST("QUDO_KEM_has_avx2()");
    int result = QUDO_KEM_has_avx2();
...
static void test_has_neon(void) {
    TEST("QUDO_KEM_has_neon()");
    int result = QUDO_KEM_has_neon();
    if (result == 0 || result == 1) {
        printf("(%s) ", result ? "Yes" : "No");
        PASS();
    } else { FAIL("unexpected value"); }
...
static void test_get_cpu_count(void) {
    TEST("QUDO_KEM_get_cpu_count()");
    int count = QUDO_KEM_get_cpu_count();
    if (count >= 1) {
        printf("(%d) ", count);
        PASS();
...
static void test_get_cache_line_size(void) {
    TEST("QUDO_KEM_get_cache_line_size()");
    size_t size = QUDO_KEM_get_cache_line_size();
...
int main(void) {
    printf("=== QUDO ML-KEM Platform Detection Tests ===\n\n");
