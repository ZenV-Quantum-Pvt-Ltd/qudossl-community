...
extern void qudo_aes_ct_encrypt(const uint8_t *in, uint8_t *out,
    ...
    || defined(_M_IX86)
#    define QUDO_AES_HAS_NI_COMPILE 1
...
extern void qudo_aes_ni_encrypt(const uint8_t *in, uint8_t *out,
                                const qudo_aes_key_t *key);
#else
#    define QUDO_AES_HAS_NI_COMPILE 0
#endif

#if QUDO_AES_HAS_NI_COMPILE
...
static int qudo_detect_aesni(void)
{
#    if defined(_MSC_VER)
    int cpuinfo[4];
    __cpuid(cpuinfo, 1);
...
static volatile int g_aes_dispatch = 0;

static void qudo_aes_init_dispatch(void)
{
    if (g_aes_dispatch != 0)
        return;

#if QUDO_AES_HAS_NI_COMPILE
    if (qudo_detect_aesni()) {
        g_aes_dispatch = 1;
        return;
    }
#endif
...
int qudo_aes_set_encrypt_key(const uint8_t *key, int bits, qudo_aes_key_t *ctx)
{
    if (!key || !ctx)
        return -1;
    ...
    qudo_aes_init_dispatch();

#if QUDO_AES_HAS_NI_COMPILE
    if (g_aes_dispatch == 1)
        return qudo_aes_ni_set_encrypt_key(key, bits, ctx);
#endif
    return qudo_aes_ct_set_encrypt_key(key, bits, ctx);
}

void qudo_aes_encrypt(const uint8_t *in, uint8_t *out,
                      const qudo_aes_key_t *key)
{
    qudo_aes_init_dispatch();
...
#if QUDO_AES_HAS_NI_COMPILE
    if (g_aes_dispatch == 1) {
        qudo_aes_ni_encrypt(in, out, key);
        return;
    }
#endif
    qudo_aes_ct_encrypt(in, out, key);
}

#ifdef QUDO_AES_ALLOW_TTABLE
...
static const u32 tt_rcon[] = {
    0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000,
    0x20000000, 0x40000000, 0x80000000, 0x1B000000, 0x36000000,
};

int qudo_aes_tt_set_encrypt_key(const uint8_t *userKey, int bits,
                                qudo_aes_key_t *key)
{
    u32 *rk;
    int i = 0;
    ...
    if (bits == 128) {
        key->rounds = 10;
        rk[0] = TT_GETU32(userKey);
        rk[1] = TT_GETU32(userKey + 4);
        rk[2] = TT_GETU32(userKey + 8);
        rk[3] = TT_GETU32(userKey + 12);
        while (1) {
            temp = rk[3];
            rk[4] = rk[0] ^ (Te2[(temp >> 16) & 0xff] & 0xff000000)
                    ^ (Te3[(temp >> 8) & 0xff] & 0x00ff0000)
                    ^ (Te0[(temp) & 0xff] & 0x0000ff00)
                    ^ (Te1[(temp >> 24)] & 0x000000ff) ^ tt_rcon[i];
            rk[5] = rk[1] ^ rk[4];
            rk[6] = rk[2] ^ rk[5];
            rk[7] = rk[3] ^ rk[6];
            if (++i == 10)
    ...
    if (bits == 192) {
        key->rounds = 12;
        rk[4] = TT_GETU32(userKey + 16);
        rk[5] = TT_GETU32(userKey + 20);
        while (1) {
            temp = rk[5];
    ...
    rk[4] = TT_GETU32(userKey + 16);
    rk[5] = TT_GETU32(userKey + 20);
    rk[6] = TT_GETU32(userKey + 24);
    rk[7] = TT_GETU32(userKey + 28);
    while (1) {
        temp = rk[7];
...
void qudo_aes_tt_encrypt(const uint8_t *in, uint8_t *out,
                         const qudo_aes_key_t *key)
{
    const u32 *rk;
    ...
    rk = key->rd_key;
    s0 = TT_GETU32(in) ^ rk[0];
    s1 = TT_GETU32(in + 4) ^ rk[1];
    s2 = TT_GETU32(in + 8) ^ rk[2];
    s3 = TT_GETU32(in + 12) ^ rk[3];
